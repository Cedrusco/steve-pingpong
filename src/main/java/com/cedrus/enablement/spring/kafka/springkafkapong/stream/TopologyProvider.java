...
import com.cedrus.enablement.spring.kafka.springkafkapong.model.PongPlayers;
  ...
  private final ObjectMapper objectMapper;
    ...
    public TopologyProvider(TopicConfig topicConfig, AppConfig appConfig, ObjectMapper objectMapper){
        ...
        public Topology getTopology(PongTarget pongTarget) {
            ...
            final KStream<String, String> playerStream = incomingStream.map((key, value) -> {
                log.info("incomingStream key before randomizer ={}", key);
                return KeyValue.pair(randomKey(), value);
            });

            final KStream<String, String> filteredStream = playerStream.branch((key, value) -> {
                log.info("playerStream key after randomizer ={}", key);
